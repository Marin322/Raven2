...
import { Statistics } from "../../../widgets/admin-sidebar/ui/Statistics";
import { AuditLogs } from "../../../widgets/admin-sidebar/ui/AuditLogs";
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "departmentEdit":
        return (
          <div>
            <EditDepartment deptData={formData} />
          </div>
        );
    ...
    <div className="w-full h-screen overflow-hidden bg-main-bg flex relative">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="md:hidden flex items-center p-4 bg-secondary-bg border-b border-border-bg">
          ...
          <h1 className="ml-4 text-main-text font-medium">Панель управления</h1>
        </header>

        <div className="flex-1 overflow-y-auto p-5 flex justify-center">
          <div className="w-full max-w-160">{renderContent()}</div>
        </div>
      </div>
    </div>
  );
